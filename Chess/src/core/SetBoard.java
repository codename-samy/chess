...
public class SetBoard {
  ...
  public SetBoard(ChessBoard chessBoard) {
    ...
    for (int i = 0; i < chessBoard.getNumberOfFiles(); i++) for (
      int j = 0;
      j < chessBoard.getNumberOfRanks();
      j++
    ) {
      String color;
      if (i % 2 == 0) {
        if (j % 2 == 0) color = "white"; else color = "black";
      } else if (j % 2 == 0) color = "black"; else color = "white";
